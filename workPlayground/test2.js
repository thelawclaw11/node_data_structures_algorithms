...
const projectId = "aa166547-9a1d-4f5c-b7b8-1e6727f71faa";
...
const main = async () => {
    ...
    const firstPage = await axios.get(`${config.base}/flow-api/projects?size=${SIZE}`, {
        headers: { cookie: config.cookie },
    });
    if (firstPage.data._embedded.projects) {
        projects.push(...firstPage?.data?._embedded.projects);
    }
    ...
    for (let i = 1; i < totalPages; i++) {
        const nextPage = await axios.get(
            `${config.base}/flow-api/projects?size=${SIZE}&page=${i}`,
            { headers: { cookie: config.cookie } }
        );

        if (nextPage?.data?._embedded?.projects) {
            projects.push(...nextPage?.data?._embedded?.projects);
        }
    }
    ...
    console.log("total items received: ", projectIds.length);
